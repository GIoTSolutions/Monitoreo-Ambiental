...
MQTT_PORT          = 1883
MQTT_KEEPALIVE     = 60
MQTT_URL_PUB       = "/v1.6/devices/eca1_giots"
MQTT_URL_TOPIC     = "/cmd"
UBIDOTS_MQTT_TOKEN = "XyMpOenz6mdldIIKEbFhOZqrkxXaP0"
#------------------------------------------------------------#
# Message structure
#------------------------------------------------------------#
if EXAMPLE_WITH_Z1:
...
def publish_recv_data(data, pubid, conn, addr):
  try:
    ...
    print "MQTT: Publishing to {0}... " + "{1}".format(mid, res)
...
def jsonify_recv_data(msg):
  sensordata = '{'
  for f_name, f_type in msg._fields_:
    sensordata += '"{0}":{1},'.format(f_name, getattr(msg, f_name))
...
def start_client():
  ...
  if ENABLE_MQTT:
    ...
    client.username_pw_set(UBIDOTS_MQTT_TOKEN)
